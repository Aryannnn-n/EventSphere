...
export async function POST(req: Request, { params }: RouteParams) {
  try {
    ...
    const allowedStatuses = ['GUEST_INVITED', 'ONGOING', 'COMPLETED'];
    if (!allowedStatuses.includes(event.status)) {
      return NextResponse.json({ error: 'Attendance can only be marked after the event is approved and scheduled' }, { status: 400 });
    }
